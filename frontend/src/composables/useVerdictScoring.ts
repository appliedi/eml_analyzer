...
export type RiskLevel = 'unknown' | 'clean' | 'low-risk' | 'suspicious' | 'high-risk' | 'malicious'
...
export function useVerdictScoring(verdicts: Ref<VerdictType[]>) {
  ...
  const maliciousCount = computed(() => scoringVerdicts.value.filter((v) => v.malicious).length)
